...
**`cx ai-center`:** AI Center (GenAI) configuration commands wrapping the AI v3 REST API (`/mgmt/openapi/5/ai/...`, proto `com/coralogixapis/ai/v3`): `applications` (list/get), `evaluations` (list/get/create/update/delete), `custom-evaluations` (list/list-for-application/create/update/add-policy/remove-policy), `count` (apps per evaluation type), and `model-pricing` (get/set). Mirrors the `ws-ai-mcp` MCP AI Center tool surface. Telemetry (GenAI spans) is queried separately via `cx spans`/`cx logs`; see the `cx-ai-center` skill. No delete for custom-eval policies, applications, or pricing — by design.
