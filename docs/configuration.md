...
In practice, `team_url` is not returned by `/identity/whoami` for every team - there is currently no public Coralogix API that reliably exposes a team's console URL slug in every case, so step 2 can silently fall through to "no link" even on a region with a known console domain. When that happens, `cx` prints a one-time (per invocation) dimmed hint on stderr pointing at `console_url` as the fix, rather than silently doing nothing or guessing from `team_name` (see point 2 above on why guessing is unsafe). Setting `console_url` explicitly is the only way to *guarantee* links today on a team whose `/identity/whoami` doesn't return `team_url`.
