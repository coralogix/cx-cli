...
**Requirements:** `cx search-fields` requires `team_id` in the active profile and an OpenAI API key (via `OPENAI_API_KEY` or profile `openai_api_key`). If these are missing, prompt the user to run `cx configure`.
