...
fn read_from_file(path: &str) -> Result<Value> {
    let raw = if path == "-" {
        ...
        eprintln!("{}", format!("Reading connector definition from {path}...").dimmed());
...
pub async fn run_list(targets: &[Arc<ExecutionTarget>], output: OutputFormat) -> Result<()> {
    ...
    match output {
        ...
        OutputFormat::Text => {
            ...
            let rows: Vec<Vec<String>> = all_items.iter().map(|(profile, conn)| {
                vec![
                    profile.clone(),
            ...
            render::render_table(&["ID", "Name", "Type", "Enabled", "Created"], rows, include_profile);
        }
    }
    Ok(())
}
...
pub async fn run_get(targets: &[Arc<ExecutionTarget>], id: &str, output: OutputFormat) -> Result<()> {
    ...
    for (profile, result) in per_profile {
        match result {
            Ok(mut val) => {
                if include_profile { render::tag_get_result(&mut val, &profile); }
    ...
    match output {
        ...
        OutputFormat::Agents => {
            let toon = toon_encode(&all_results).map_err(|e| anyhow::anyhow!("TOON encoding failed: {e}"))?;
            println!("{toon}");
        }
        OutputFormat::Text => {
            render::render_get_text(&all_results, include_profile, "Connector not found.", None::<&dyn Fn(&Value)>)?;
        }
    }
    Ok(())
}
        ...
        match result {
            Ok(resp) => {
                if let Some(conn) = resp.connector {
                    ...
                    eprintln!("{}", format!("Created connector '{name}' (ID: {id}) in profile '{profile}'.").green());
...
pub async fn run_update(targets: &[Arc<ExecutionTarget>], from_file: &str, output: OutputFormat) -> Result<()> {
    ...
    for (profile, result) in per_profile {
        match result {
            Ok(val) => { eprintln!("{}", format!("Updated connector in profile '{profile}'.").green()); all_results.push(val); }
            Err(e) => eprintln!("{}", format!("error from profile '{profile}': {e:#}").red()),
        }
    }
...
pub async fn run_delete(targets: &[Arc<ExecutionTarget>], id: &str) -> Result<()> {
    ...
    let per_profile = fan_out(targets, |t| {
        let id = id.clone();
        async move { let api = ConnectorsApi::new(&t.client); api.delete(&id).await?; Ok(()) }
    }).await;
    for (profile, result) in per_profile {
        match result {
            Ok(()) => eprintln!("{}", format!("Connector {id} deleted in profile '{profile}'.").green()),
            Err(e) => eprintln!("{}", format!("error from profile '{profile}': {e:#}").red()),
        }
    }
    Ok(())
...
pub async fn run_types(targets: &[Arc<ExecutionTarget>], output: OutputFormat) -> Result<()> {
    ...
    match output {
        ...
        OutputFormat::Agents => { let toon = toon_encode(&all_results).map_err(|e| anyhow::anyhow!("TOON encoding failed: {e}"))?; println!("{toon}"); }
        OutputFormat::Text => {
            for val in &all_results { println!("{}", serde_json::to_string_pretty(val)?); }
        }
    }
    Ok(())
}
