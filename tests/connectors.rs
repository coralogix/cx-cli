...
use cx::commands::connectors::{run_list, run_types};
...
async fn list_connectors_from_mock() {
    ...
    Mock::given(method("GET"))
        .and(path("/mgmt/openapi/latest/notifications/connectors/v1"))
...
async fn get_connector_types_from_mock() {
    ...
    Mock::given(method("GET"))
        .and(path("/mgmt/openapi/latest/notifications/connectors/v1/types"))
    ...
    let target = common::test_target("test-profile", &server.uri());
    run_types(&[target], OutputFormat::Json).await.expect("run_types should succeed");
}
